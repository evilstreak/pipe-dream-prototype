...
class TileRack
  ...
  def initialize(window, top_left, bottom_right, tile_count)
    ...
    @tiles = build_tiles(window, top_left, bottom_right, tile_count)
  ...
  def build_tiles(window, top_left, bottom_right, tile_count)
    ...
    tile_count.times.map do |index|
      Tile.new(window, point.offset(0, offset * index), 96)
    end
  end
end
